...
class CareApi
{
    ...
    public function getGuestPageUrl($reservationId = false)
    {
        
        ////////////////////////
        // get the administration so we can redirect the user back to the guest page
        ////////////////////////

        $responseAdministration = $this->get('/administrations/' . Session::get('adminId'));

        if($reservationId){
            $responseReservation = $this->get('/reservations/' . $reservationId, ["get_meta" => true]);
            $reservationMeta = $responseReservation->json('meta') ;
            return 'https://'.$responseAdministration->json('slug').'.camping.care/reservation/'.$reservationId.'/'.$reservationMeta['hash'] ;
        }
        
        return 'https://booking.camping.care/'.$responseAdministration->json('slug');
